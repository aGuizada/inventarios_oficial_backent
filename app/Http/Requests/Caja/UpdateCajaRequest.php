...
class UpdateCajaRequest extends FormRequest
{
    ...
    {
        $param = $this->route('caja');
        $caja = $param instanceof Caja ? $param : Caja::find($param);

        if (! $caja) {
            throw new NotFoundHttpException('Caja no encontrada.');
        }

        return $this->user() !== null && $this->user()->can('update', $caja);
    }
